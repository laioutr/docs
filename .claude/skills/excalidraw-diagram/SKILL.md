...
You cannot judge a diagram from JSON alone. After generating or editing the Excalidraw JSON, you MUST render it, view the result, and fix what you see — in a loop until it's right. This is a core part of the workflow, not a final check.
...
This outputs an SVG next to the `.excalidraw` file (default). Use `--format png` for a rasterized version. To view the result, render a PNG and use the **Read tool** on it:

```bash
cd .claude/skills/excalidraw-diagram/references && uv run python render_excalidraw.py <path-to-file.excalidraw> --format png
```
...
**1. Render & View** — Run the render script with `--format png`, then Read the PNG.
...
**5. Re-render & re-view** — Run the render script again and Read the new PNG. Once the diagram is finalized, render the final version as SVG (the default) for production use.
...
21. **Rendered and inspected**: Diagram has been rendered (PNG for review, SVG for production) and visually inspected
...
26. **Readable at export size**: Text is legible in the rendered output
