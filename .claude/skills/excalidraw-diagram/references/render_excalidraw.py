...
Usage:
    ...
    uv run python render_excalidraw.py <path-to-file.excalidraw> [--output path.svg] [--scale 2] [--width 1920]
    uv run python render_excalidraw.py <path-to-file.excalidraw> --format png

Output format is determined by: --format flag > output file extension > default (svg).
...
def strip_embedded_fonts(svg: str) -> str:
    ...
    for name, generic in _FONT_REPLACEMENTS.items():
        svg = svg.replace(name, generic)
...
def render(
    ...
    """Render an .excalidraw file to SVG or PNG. Returns the output path."""
    ...
    # Determine output format: explicit flag > output extension > default svg
    if fmt is None:
    ...
    if fmt not in ("svg", "png"):
        print(f"ERROR: Unsupported format '{fmt}'. Use 'svg' or 'png'.", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        ...
        if fmt == "svg":
            # Extract the serialized SVG markup directly
            svg_markup = page.evaluate("window.__svgMarkup")
            if not svg_markup:
                print("ERROR: No SVG markup captured after render.", file=sys.stderr)
                browser.close()
                sys.exit(1)
            svg_markup = strip_embedded_fonts(svg_markup)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render Excalidraw JSON to SVG or PNG")
    parser.add_argument("input", type=Path, help="Path to .excalidraw JSON file")
    parser.add_argument("--output", "-o", type=Path, default=None, help="Output path (default: same name with .svg)")
    parser.add_argument("--format", "-f", choices=["svg", "png"], default=None, help="Output format (default: svg). Overrides output extension.")
    ...
    out_path = render(args.input, args.output, args.format, args.scale, args.width)
    print(str(out_path))
