...
links: []
seo:
  title: Standard Customer Support | Laioutr
sitemap:
  loc: /offering/customer-support/standard-customer-support
  lastmod: 2026-04-08
  changefreq: monthly
  priority: 1.0
...
## We're here to help you get everything out of Laioutr's Frontend Management Platform
...
::note
For optional prioritized handling, shorter response time targets, and phone callback, we will provide a Premium Support offering in the future.
::
...
Support request handling priority is set by Laioutr, respecting Severity Level and contractual status.
...
**Email:** Email is only to be used in case the Support website is unavailable. Support requests are submitted via email: <support@laioutr.com>

- for severity 3 and 4 requests <support@laioutr.com> and for severity 1 and 2 requests <incident@laioutr.com>
...
- Every Customer must provide one 24/7 reachable escalation email contact address for Laioutr to be able to reach out to resolve Incidents that require Customer actions.
- Laioutr must be enabled to reproduce reported problems. Customers and partners must cooperate closely and timely with suitably skilled staff to enable reproduction. Laioutr Support may need to be granted access to the Customers' organizations for troubleshooting purposes. One of the Customer’s users participating in a Support request must have required system permissions to change system configuration or permissions.
- Laioutr can request the Customer to join a screen-sharing conference for interactive problem resolution at the discretion of Laioutr. The Customer has to join with suitably skilled staff.
- If Laioutr provides the Customer with an updated version of its Apps or other client-side software free of charge to resolve the problem, the Customer must make efforts to apply it.
...
- solutions and integrations from the Laioutr App Store (please contact the solution provider)
