...
2. **Initialise** – **init()** loads the CMP script, sets up cookies, and subscribes to the provider’s consent events. When the user changes consent in the CMP, the adapter should call the callbacks registered via **onConsentChange** with a **Partial&lt;ConsentManagementState&gt;** so the store stays in sync.
3. **Report current state** – **getConsentState()** returns the current consent (from cookie or CMP API) in the same five-category shape (or a partial). Can be sync or async.
4. **Expose UI triggers** – **showConsentOverlay()** and **renewConsent()** should open the CMP banner or preferences dialog.
5. **Answer category checks** – **hasCategoryConsent(category)** returns whether the given category is granted (typically derived from **getConsentState()**).
6. **Optional cleanup** – **destroy()** can tear down listeners or resources when the adapter is deactivated.
...
- Laioutr’s **consent management abstraction** is a single store and a small **ConsentAdapter** interface in **@laioutr-core/frontend-core**. Consent is represented by five categories: necessary, functional, statistics, marketing, unclassified.
- Use **useConsentStore()** to register/activate adapters, check **hasCategoryConsent(category)**, and call **showConsentOverlay()** or **renewConsent()**. Other apps (e.g. GTM, tracking) read from the same store so tags and events respect consent.
- To add your own CMP, implement a **ConsentAdapter** (types from **#frontend/consent**), map your provider’s categories to **ConsentManagementState**, and register/activate the adapter in a client plugin.
- For a ready-to-use solution, use the **@laioutr-app/cookiebot** app and follow the **[Cookiebot app documentation](/apps/app-docs/cookiebot)** for setup and configuration.
