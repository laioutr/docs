---
title: Media
description: The discriminated union returned anywhere a connector or component schema yields an image or video. Two variants, source-driven rendering through the Media component.
seo:
  title: Media | Laioutr
  description: The discriminated union returned anywhere a connector or component schema yields an image or video. Two variants, source-driven rendering through the Media component.
sitemap:
...
A product has images. A hero banner has a background image. A blog post has a featured image. A category card might have a video. The shape connectors and editors return for all of these is the same `Media` discriminated union: a `type` field plus a list of source URLs (one per variant the renderer can pick from). The storefront's [`<Media>`](/laioutr-ui/ui-kit/general/media) component reads the sources, picks the right one for the current viewport, and hands the chosen URL to nuxt-image so the right provider can serve it.
...
`provider` is the most load-bearing field. It tells nuxt-image which adapter to use to fetch and resize this URL. Common values are `'shopify'` (Shopify CDN), `'cloudinary'`, `'directus'`, `'sanity'`, custom names registered by your app, or `'raw'` for plain URLs that need no transformation.
...
::warning
nuxt-image is configured with `provider: 'none'` at the global level. Each source's `provider` field decides which adapter handles that URL. If a source uses a provider name your storefront does not have configured, nuxt-image falls back to serving the URL as-is (which still works for plain URLs but skips resizing and format optimization).
::
...
> Map your backend's asset shape to a `Media` object with the right `provider`. Never return a bare URL string, never default everything to `'raw'`, and never drop the dimensions when the backend gives them to you.
...
| A plain CDN URL with no resize API | `'raw'`                    | Skips optimization but works                                                                  |

Picking `'raw'` everywhere "to keep things simple" gives up the bandwidth savings and format conversion that nuxt-image is there to do. Spend the time wiring up the right provider; it pays back on every page load.
