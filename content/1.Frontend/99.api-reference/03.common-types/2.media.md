...
| [`video`](#mediavideo) | Product demos, hero loops, autoplay backgrounds             | `<video>` with sources (no built-in player UI yet)         |
| [`audio`](#mediaaudio) | Podcasts, voice-overs, music previews                       | `<audio>` with sources (no built-in player UI yet)         |
...
The Media UI Kit component does not render `MediaAudio` today. Render it directly with an `<audio>` element keyed off `media.sources`.
