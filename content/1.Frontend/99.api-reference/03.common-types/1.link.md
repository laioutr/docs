...
To override resolution behavior (for example, to route certain reference types through a custom path), register a [link resolver hook](/frontend/features/hooks#link-resolver). When the hook sets `result.value`, the default resolution is skipped entirely.
