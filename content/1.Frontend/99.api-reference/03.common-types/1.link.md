...
To post-process the resolved URL (for example, append a query parameter), register a [link resolver hook](/frontend/features/hooks#link-resolver). The hook runs after resolution with `result.value` pre-seeded with the resolved value; your handler can read and transform it.
