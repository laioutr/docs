...
Some shapes recur across many entities, queries, and actions: the `Link` discriminated union returned by breadcrumb items, menu items, and CTA fields; the `Media` union returned wherever an entity exposes images or videos; the `Money` record carried by every price, shipping rate, discount, and cart total; the `Measurement` record behind dimensions, weights, volumes, and per-unit pricing. Browse these in the [Common Types](/frontend/api-reference/common-types) section.
