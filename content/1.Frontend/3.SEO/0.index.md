...
- **Canonical URL shape** – A project-wide [trailing-slash policy](/frontend/features/routing#trailing-slashes) keeps canonical and hreflang links in one shape and 301-redirects the other, so a page is only ever indexed at one address.
- **Locale-correct alternates** – On a page type whose slug is [translated per locale](/frontend/features/multi-language-support#translated-slugs), each hreflang alternate points at that locale's own slug, and locales where the page does not exist are left out.
