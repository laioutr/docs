...
Laioutr ships **Orchestr Devtools** as a Nuxt module that adds two dedicated tabs to Nuxt Devtools: one for queries and one for actions. Add `@laioutr-core/orchestr-devtools` to `modules` in your `nuxt.config.ts` to enable them. The [app-starter playground](https://github.com/laioutr/app-starter/blob/main/playground/nuxt.config.ts) already includes it.
