...
When Claude Code writes a Section, wires up an Orchestr handler, or builds a hero component from a Figma frame, you want it following the same patterns the Laioutr core team applies in the monorepo: schema field naming, the `ui-kit` / `ui` / `ui-app` layering, `--z-index-*` tokens, currency formatting through canonical types. The **Laioutr developer plugin** ships those workflows as Claude Code skills, plus the [docs MCP server](/getting-started/documentation-mcp-server) wired up out of the box, so any project gets the same defaults from a single install.
...
The plugin registers the `laioutr-docs` HTTP server at `https://docs.laioutr.com/mcp` automatically. That is the same server documented at [Documentation MCP Server](/getting-started/documentation-mcp-server).
