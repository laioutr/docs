...
- Outbound HTTPS to `https://docs.laioutr.io` so the docs MCP server can connect.
...
The plugin registers the `laioutr-docs` HTTP server at `https://docs.laioutr.io/mcp` automatically. That is the same server documented at [Documentation MCP Server](/getting-started/mcp-server).
