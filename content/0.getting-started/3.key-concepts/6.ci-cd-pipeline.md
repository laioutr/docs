...
```mermaid
flowchart LR
  Dev[Developer workstation – local Laioutr app repo] --> A[Implement or update Nuxt module, sections, blocks, orchestr]
  A --> B[Run local playgrounds – Frontend Core and Studio]
  B --> C[Commit and push to VCS (GitHub, GitLab, …)]

  C --> CI[Customer CI/CD pipeline]
  CI --> CI_Build[Build, lint, test app]
  CI_Build --> CI_Publish[Version bump and publish npm package]
  CI_Publish --> Registry[(Package registry – npm or private)]

  Registry --> FE_Deps[Update Laioutr Frontend dependencies and laioutrrc.json]
  FE_Deps --> FE_Local[Regenerate local frontend with CLI and test]
  ...
  FE_Deploy --> Users[End users and editors (Frontend and Studio)]
```
