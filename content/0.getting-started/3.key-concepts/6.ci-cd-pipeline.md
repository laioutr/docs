...
This page explains, at a high level, how developers ship changes from their local machine into a **Laioutr frontend** using **Nuxt modules** (Laioutr apps) published as **packages** (for example to **npm**).  
It focuses on:
...
```mermaid
flowchart LR
  Dev[Developer workstation<br/>Local Laioutr app repo] --> A[Implement or update Nuxt module<br/>+ sections, blocks, orchestr]
  A --> B[Run local playgrounds<br/>Laioutr Frontend Core + Studio]
  B --> C[Commit & push to VCS<br/>(e.g. GitHub, GitLab)]

  C --> CI[Customer CI/CD pipeline]
  CI --> CI_Build[Build, lint, test app]
  CI_Build --> CI_Publish[Version bump & publish<br/>npm package]
  CI_Publish --> Registry[(Package registry<br/>e.g. npm / private registry)]

  Registry --> FE_Deps[Update Laioutr Frontend deps<br/>(package.json + laioutrrc.json)]
  FE_Deps --> FE_Local[Regenerate local frontend via CLI<br/>and test in browser + Studio]

  FE_Deps --> FE_CI[Frontend CI/CD pipeline]
  FE_CI --> FE_Build[Build Laioutr frontend]
  FE_Build --> FE_Deploy[Deploy to hosting environment]
  FE_Deploy --> Users[End users & editors<br/>(Frontend + Studio)]
```
...
- **Start from the App Starter**  
  Use the official [App Starter](/apps/app-development/app-starter) to create a new Laioutr app.  
  This gives you:
...
- **Treat the app as a package from day one**  
  In `package.json`, the `name` field defines your **package identity** (for example `@your-org/laioutr-app-example`).  
  Laioutr uses the same name as the **config key** when wiring configuration from `laioutrrc.json` into your module.

- **Implement frontend and orchestr pieces**  
  Add or modify:
...
- **Connect Studio to local frontend**  
  When supported by your project configuration, use Studio’s developer options (for example a “use localhost” command) so:
...
- **Regenerate the local Laioutr frontend via CLI (future)**  
...
Each customer (or team) owns their **integration pipeline** for their apps.  
...
- **Frontend project dependencies**  
  In the Laioutr frontend repository:
...
- **Project configuration (`laioutrrc.json`)**  
  In `laioutrrc.json`:
...
2. **Generate frontend code (CLI)**  
   Run the Laioutr CLI command to regenerate the frontend from `laioutrrc.json` and installed apps.  
...
4. **Deploy**  
   Deploy the built frontend to your hosting provider or edge platform.  
   Once live:
