...
seo:
  title: Architecture | Laioutr
  description: Understand how a Laioutr frontend is structured, from the visual editor down to the Nuxt application that powers it.
sitemap:
  loc: /getting-started/key-concepts/architecture
  lastmod: 2026-04-08
  changefreq: monthly
  priority: 1
---
...
to: /getting-started/next-steps/create-custom-app
---
...
flowchart TD
  ...
  P --> S2["Section: Two-Column Feature"]
  S1 --> SL1["Slot: content"]
  S2 --> SL2["Slot: left"]
  S2 --> SL3["Slot: right"]
  SL1 --> B1["Block: Headline"]
  SL1 --> B2["Block: CTA Button"]
  SL2 --> B3["Block: Image"]
  SL3 --> B4["Block: Text"]
```

- **Sections** are the top-level layout units on a page (hero banner, product grid, footer). They define structure and spacing, and expose named **slots** where blocks can be placed.
- **Slots** are named children of a section — they map directly to Vue slots. Each slot can optionally restrict which blocks are allowed in it.
- **Blocks** are the discrete UI elements placed inside a section's slots (a headline, a CTA button, a product tile). Blocks themselves do not expose slots.

Studio lets non-developers compose pages by arranging sections and filling their slots with blocks. Developers define the available sections, their slots, and blocks when building apps.
